...
public:
	...
	T_Entry<K, V>& operator[](int index) { return t_array[index]; } // 배열의 원소 반환
...
void TA_Entry<K, V> :: insert(int i, T_Entry<K, V> element)
{
	...
	if (isValidIndex(i))
	{
		...
		t_array[i] = element; // t_array[i]에 element을 삽입
		num_elements++;
	}
}
template<typename K, typename V>
void TA_Entry<K, V> ::remove(int i)
{
	if (isValidIndex(i))
	{
		for (int j = i + 1; j < num_elements; j++)
			t_array[j - 1] = t_array[j]; // 배열원소들을 하나씩 내리기
		num_elements--;
	}
...
int TA_Entry<K, V> ::sequential_search(T_Entry<K, V> search_key)
{
	...
	{
		if (t_array[index] == search_key) // 일치한다면 인덱스 반환
			return index;
	}
	return -1;
}
...
int TA_Entry<K, V> ::binary_search(T_Entry<K, V> search_key)
{
	...
	while (low <= high)
	{
		...
		mid = (low + high) / 2;
		if (t_array[mid] == search_key) // 일치한다면 mid반환
			return mid;
